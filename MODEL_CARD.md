...
Baseline Labs is research software for exploring tennis forecasts. It supports five workflows:
...
4. current tournament draws whose completed results and unresolved matchups are refreshed from a live scoreboard; and
5. source-visible match context covering availability reporting, injuries, coaching changes, venue conditions, recovery, and recent travel.

It is not validated for wagering, player health decisions, selection decisions, or any other high-stakes use.

## Architecture

The browser model has six layers:
...
- **Context intelligence.** Current weather and elevation can alter serve conditions or widen uncertainty. A 21-day match window estimates recovery time and travel distance. Recent injury, withdrawal, recovery, and coaching-change headlines become visible evidence signals. Context changes the latent player inputs before simulation; it does not edit a final percentage after the fact.
...
Context adjustments are deliberately bounded and asymmetric:

- one unverified adverse headline widens uncertainty but creates no directional rating penalty;
- independently corroborated reporting can create a bounded availability or injury adjustment;
- a trusted tournament, tour, or major news source can confirm a withdrawal;
- confirmed withdrawal from the selected event produces a walkover rather than a simulated match;
- return-to-play and speculative headlines remain directionally neutral;
- coaching changes widen uncertainty because the model does not assume a new coach is inherently better or worse;
- travel penalties require both meaningful distance and a short recovery window; and
- outdoor weather affects the serve posterior or uncertainty only at material thresholds. Likely indoor events do not receive outdoor weather adjustments.

Every applied factor, source headline, confidence label, and rating/uncertainty change is shown in the interface.
...
- Travel estimates can miss lower-tour or otherwise absent matches and approximate time-zone shift from longitude rather than a player's actual itinerary.
- Weather is a location-level model forecast. Court shade, roof closure, indoor climate control, ball changes, and hyperlocal wind are not directly observed.
- Context availability is automatic for same-tour professional 1v1 forecasts and on demand for individual live-draw matchups. Custom fictional entrants cannot be linked reliably to news or travel history.
...
The next model milestone is a rolling-origin backtest that records log loss, Brier score, calibration curves, and accuracy against ranking-only, overall-Elo, surface-Elo, and context-ablation baselines. Results should be versioned by data cutoff and model commit.
